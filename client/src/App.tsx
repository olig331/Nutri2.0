...
import { UserSetUp } from "./components/CreateNewUser/UserSetUp";
...
import { UserSelect } from "./components/Login/UserSelect";
...
import { ForgotUserName } from "./components/ForgotDetails/ForgotUserName/ForgotUserName";
import { ForgotPassword } from "./components/ForgotDetails/ForgotPassWord/ForgotPassword";
import "./style/style.css";
import { CustomWindowToolBar } from "./components/WindowToolBar/CustomWindowToolBar";
...
export const App: React.FC = () => {
	...
	const [navigatedFromLogin, setnavigatedFromLogin] = useState<boolean>(
		false
	);
	...
	] = useState<UsersDBType>();
	const [dailyFood, setdailyFood] = useState<any[]>([]);
	const [loggedInID, setloggedInID] = useState<string>("");
	const [navigatedFromTracker, setnavigatedFromTracker] = useState<boolean>(
		false
	);
	...
	return (
		<div className="app">
			<NavigatedFromLoginContext.Provider
				value={{ navigatedFromLogin, setnavigatedFromLogin }}
			>
				...
				>
					<NavigatedFromTrackerContext.Provider
						value={{
							navigatedFromTracker,
							setnavigatedFromTracker,
						}}
					>
						...
						>
							...
							>
								<DailyFoodContext.Provider
									value={{ dailyFood, setdailyFood }}
								>
									<LoggedInIDContext.Provider
										value={{ loggedInID, setloggedInID }}
									>
										<CustomWindowToolBar />
										<SignedOutContext.Provider
											value={{ signedOut, setsignedOut }}
										>
											<Router>
												<Switch>
													<Route
