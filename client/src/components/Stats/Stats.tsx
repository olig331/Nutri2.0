	...
	const usersCalculatedIntake = (): number => {
		...
		if (stats.gender === "male") {
			bmr =
				...
				13.397 * weightInKg +
				4.799 * heightInCm -
				5.677 * parseInt(stats.age);
		} else {
			bmr =
				447.593 +
				9.247 * weightInKg +
				3.098 * heightInCm -
				4.33 * parseInt(stats.age);
		}

		cals = Math.floor(
			stats.activityLevel === "sedatory"
				? (bmr *= 1.2)
			...
			<div className="all_stats">
				<>
					<div className="bar">
						<h5>
							Calories:{" "}
							{Math.round(getNutrientTotal("nf_calories"))} -{" "}
							{helperFunc("calories")} KCal
						</h5>
						<ProgressBar
							percent={getNutrientPercent("nf_calories")}
						/>
					</div>
					...
					<div className="bar">
						<h5>
							Protein:{" "}
							{Math.round(getNutrientTotal("nf_protein"))}g -{" "}
							{helperFunc("nf_protein")}g
						</h5>
						<ProgressBar
							percent={getNutrientPercent("nf_protein")}
						/>
					</div>
					...
					<div className="bar">
						<h5>
							Carbohydrates:{" "}
							{Math.round(
								getNutrientTotal("nf_total_carbohydrate")
							)}
							g - {helperFunc("nf_total_carbohydrate")}g{" "}
						</h5>
						<ProgressBar
							percent={getNutrientPercent(
					...
					<div className="bar">
						<h5>
							Fat: {Math.round(getNutrientTotal("nf_total_fat"))}g
							- {helperFunc("nf_total_fat")}g
						</h5>
						<ProgressBar
							percent={getNutrientPercent("nf_total_fat")}
						/>
					</div>
					...
					<div className="bar">
						<h5>
							Saturated Fat:{" "}
							{Math.round(getNutrientTotal("nf_saturated_fat"))}g
							- {helperFunc("nf_saturated_fat")}g
						</h5>
						<ProgressBar
							percent={getNutrientPercent("nf_saturated_fat")}
						/>
					</div>

					<div className="bar">
						<h5>
							Sugars: {Math.round(getNutrientTotal("nf_sugars"))}g
							- {helperFunc("nf_sugars")}g
						</h5>
						<ProgressBar
							percent={getNutrientPercent("nf_sugars")}
						/>
					</div>
				</>
			</div>
